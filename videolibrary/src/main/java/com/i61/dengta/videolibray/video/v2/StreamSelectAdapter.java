...
public class StreamSelectAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = this.layoutInflater.inflate(R.layout.v2_simple_player_stream_list_item, null);
            holder = new ViewHolder();
            holder.streamName = convertView.findViewById(R.id.v2_simple_player_stream_name);
        ...
        if (mVideoijkBean.isSelect()) {
            holder.streamName.setTextColor(mContext.getResources().getColor(R.color.stream_name_playing));
        } else {
            holder.streamName.setTextColor(mContext.getResources().getColor(R.color.stream_name_normal));
        }
        return convertView;
    }

    public void update(List<VideoijkBean> list) {
        listVideos = list;
